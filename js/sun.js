...
var starColorGraph = THREE.ImageUtils.loadTexture( '../images/sun/star_color_modified.png' );

function loadStarSurfaceTextures(){
	if( sunTexture === undefined ){
		sunTexture = THREE.ImageUtils.loadTexture( "../images/sun/sun_surface.png", undefined );
		//sunTexture.anisotropy = maxAniso;
		sunTexture.wrapS = sunTexture.wrapT = THREE.RepeatWrapping;
	}

	if( sunColorLookupTexture === undefined ){
		sunColorLookupTexture = THREE.ImageUtils.loadTexture( "../images/sun/star_colorshift.png" );
	}

	if( solarflareTexture === undefined ){
		solarflareTexture = THREE.ImageUtils.loadTexture( "../images/sun/solarflare.png", undefined	 );
	}

	if( sunHaloTexture === undefined ){
		sunHaloTexture = THREE.ImageUtils.loadTexture( "../images/sun/sun_halo.png", undefined );
	}

	if( sunHaloColorTexture === undefined ){
		sunHaloColorTexture = THREE.ImageUtils.loadTexture( "../images/sun/halo_colorshift.png" );
	}

	if( sunCoronaTexture === undefined ){
		sunCoronaTexture = THREE.ImageUtils.loadTexture( "../images/sun/corona.png", undefined);
	}
}
...
function makeSun( options ){
	var radius = 7.35144e-8;//options.radius;
	var spectral = 0.656//options.spectral;
  ...
  // console.timeEnd("make sun solarflare");
	...
	sun.update = function(){
		...
		if( camera.position.z > 400 ){
			var lensflareChild = this.gyro.getChildByName('lensflare');
			if( lensflareChild !== undefined )
				this.gyro.remove(  lensflareChild );
		}
		else{
			if( this.gyro.getChildByName('lensflare') === undefined ){
				this.gyro.add( this.lensflare );			
			}
